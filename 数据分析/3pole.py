...
# 提取输入（器件参数）和输出（实极点）：保留原始顺序，不做排序
X = df[['Cs', 'Rd', 'Rs']].values  # 3个输入特征
y = df[['Pole1_Real', 'Pole2_Real', 'Pole3_Real']].values  # 3个实极点输出（均为负，保留原始顺序）
print("输出极点保留原始顺序（Pole1→Pole2→Pole3），不做排序")
...
class PolePredictor(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            # 输出层：32→3（对应原始顺序的Pole1、Pole2、Pole3）
            nn.Linear(32, 3)
        )
...
# 在测试集上预测并反标准化（恢复原始负极点）
with torch.no_grad():
    y_pred_scaled = model(X_test_tensor)
    # 反标准化步骤：1. 先反标准化到偏移后的正值 → 2. 减去偏移量恢复原始负极点
    y_pred_offset = scaler_y.inverse_transform(y_pred_scaled.numpy())  # 步骤1：反标准化到偏移尺度
    y_pred = y_pred_offset - output_offset_loaded  # 步骤2：减去偏移量，恢复原始负极点（保留Pole1→Pole2→Pole3顺序）
    # 真实值反标准化（同样需偏移逆操作）
    y_true_offset = scaler_y.inverse_transform(y_test_scaled)
    y_true = y_true_offset - output_offset_loaded

# -------------------------- 7. 计算评估指标（对应原始极点顺序） --------------------------
# 计算每个原始顺序极点的平均绝对误差（MAE）
mae_pole1 = np.mean(np.abs(y_pred[:, 0] - y_true[:, 0]))  # 原始Pole1的MAE
mae_pole2 = np.mean(np.abs(y_pred[:, 1] - y_true[:, 1]))  # 原始Pole2的MAE
mae_pole3 = np.mean(np.abs(y_pred[:, 2] - y_true[:, 2]))  # 原始Pole3的MAE
total_mae = (mae_pole1 + mae_pole2 + mae_pole3) / 3  # 总平均MAE

print(f"\n📊 模型评估结果（平均绝对误差MAE，对应原始极点顺序）：")
print(f"   - 原始Pole1：{mae_pole1:.4f}")
print(f"   - 原始Pole2：{mae_pole2:.4f}")
print(f"   - 原始Pole3：{mae_pole3:.4f}")
...
plt.title('训练与测试损失曲线（预测原始顺序极点）', fontsize=13, pad=15)
...
# 8.2 绘制前50个样本的预测值vs真实值对比（分原始顺序极点）
...
for i, ax in enumerate(axes):
    ...
    # 标题明确对应原始Pole1/Pole2/Pole3
    ax.set_title(f'原始Pole{i + 1}预测值与真实值对比（前{sample_num}个样本，均为负）', fontsize=12, pad=12)
...
# -------------------------- 9. 打印前10条预测结果明细 --------------------------
print("\n" + "=" * 90)
print("前10条样本预测结果明细（保留原始极点顺序：Pole1→Pole2→Pole3，均为负）")
print("=" * 90)
print(
...
print("\n" + "=" * 80)
print("所有文件已保存至当前目录：")
print("1. best_pole_predictor.pth → 最佳模型参数文件（预测原始顺序极点）")
print("2. best_model_info.npy → 最佳模型训练信息（含偏移量+标准化参数）")
print("3. loss_curve.png → 训练/测试损失曲线（标注原始极点预测）")
print("4. pole_prediction_comparison.png → 极点预测对比图（分原始Pole1/Pole2/Pole3）")
print("=" * 80)